										...
										echo "<tr class='permission_row'>";
											...
											foreach($selectedUserGroups as $group) {
												echo "<td style='min-width:100px;' class='text-center p-0' title='".$group['name']." (Click to change permission)'>";
													...
													echo $this->Html->link($img, ['action'=>'changeGroupPermission', $row['controller'], $action, $group['id'], $plugin, $prefix], array('escape'=>false, 'class'=>'change_permission'));

													echo "<br/><span style='color:green;font-size:10px; ".$inheritDisplay."' class='permission_type'>(Inherit)</span><input type='hidden' name='inherit_permission' class='inherit_permission' value='".$inheritPermission."'>";
												echo "</td>";
											}
										echo "</tr>";
									}
...
<script type="text/javascript">
	...
	$(function() {
		...
		$(document).on('click', '.permission_row .change_permission', function(e) {
			...
			var $pelem = $(this).closest('td');
			$($pelem).find('.permission_type').hide();
			...
			$.ajax({
				...
				success : function (result, textStatus, jqXHR) {
					$pelem = $(this).closest('td');
					$inheritper = $($pelem).find('.inherit_permission').val();
					...
					if(result == $inheritper) {
						$($pelem).find('.permission_type').show();
					}
				},
