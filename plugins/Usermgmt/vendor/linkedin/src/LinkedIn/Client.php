...
class Client {
	 ...
	 */
	public $authorizationUrl = "https://www.linkedin.com/oauth/v2/authorization";
	public $accessTokenUrl = "https://www.linkedin.com/oauth/v2/accessToken";
	...
	public function fetch($resource) {
		...
		$payload = array();
		$type = array('Connection: Keep-Alive', 'Authorization: Bearer '.$this->getAccessToken());

		return $this->_request($url, $payload, 'GET', $type);
	}
	 ...
	 * @return string
	 */
	public function getProfilePic($pictureData) {
		if(!empty($pictureData['displayImage~']['elements'])) {
			$element = end($pictureData['displayImage~']['elements']);

			if(!empty($element['identifiers'][0]['identifier'])) {
	 ...
	 * @return string
	 */
	public function getAuthorizationUrl($redirect_uri, $scope = null, $state = 'NOSTATE') {
		$params = array(
			'response_type' => 'code',
	...
	protected function _request($url, array $payload = array(), $method = 'GET', $type = array()) {
		$url2 = $url;
		$ch = $this->getCurl();
		if(!empty($payload['oauth2_access_token'])){
			$url = $url.'?oauth2_access_token='.$payload['oauth2_access_token'];
			unset($payload['oauth2_access_token']);
		}
